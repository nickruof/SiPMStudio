...
def _output_to_h5file(data_file, output_name, output_path, events, waveforms, channel, digitizer):
    bias = data_file["bias"]
    destination = os.path.join(output_path, f"raw_{output_name}_{bias}.h5")
    with h5py.File(destination, "w") as output_file:
        output_file.create_dataset(f"/raw/{channel}/timetag", data=events.T[0])
